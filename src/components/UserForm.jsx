...
export default function UserForm({ mode, user, pageMode = 'empleados', commercials = [], onSaved, onCancel, gridLayout = false }) {
  ...
  const handleFileSelect = (e) => {
    ...
    if (!file) return;
    console.log('[UserForm] File selected:', file.name, file.type, file.size);
    const url = URL.createObjectURL(file);
    console.log('[UserForm] Blob URL:', url);
    setCropImageUrl(url);
  ...
  const handleCropComplete = (croppedFile) => {
    console.log('[UserForm] Crop complete, file:', croppedFile.size, 'bytes, type:', croppedFile.type);
    if (previewUrl && previewUrl.startsWith('blob:')) {
      URL.revokeObjectURL(previewUrl);
    }
    const blobUrl = URL.createObjectURL(croppedFile);
    console.log('[UserForm] Preview blob URL:', blobUrl);
  ...
  const handleRestoreOriginal = () => {
    ...
    console.log('[UserForm] Restoring original photo:', originalPhotoUrl);
    if (previewUrl && previewUrl.startsWith('blob:')) {
      URL.revokeObjectURL(previewUrl);
    }
